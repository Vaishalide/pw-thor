...
<head>
  ...
  <style>
    ...
    body { margin:0; background:var(--bg); color:var(--fg); font-family:Arial, sans-serif; display:flex; flex-direction:column; height:100vh; }
    ...
    .controls { position:absolute; bottom:0; left:0; right:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; padding:4px; gap:4px; transition:opacity .3s; }
    ...
    .settings-panel { position:absolute; bottom:calc(100% + 4px); right:4px; background:var(--bg); border:1px solid var(--seek-bg); border-radius:4px; display:none; font-size:.75em; z-index:10; }
    .settings-panel.active { display:block; }
    .settings-panel button { background:none; border:1px solid var(--seek-bg); padding:2px 4px; margin:2px 1px; cursor:pointer; border-radius:3px; }
...
<body data-theme="dark">
  ...
  <div class="player" id="player">
    ...
    <div class="center-controls" id="centerControls">
      <button id="rewind10" class="btn-center">⏪ 10s</button>
      <button id="centralPlayPause" class="btn-center">❚❚</button>
      <button id="forward10" class="btn-center">10s ⏩</button>
    </div>
  ...
  <script>
      ...
      Array.from(levels.children).forEach(btn => {
        btn.classList.toggle('active',
          btn.textContent !== 'Auto'
            ? currentUrl.endsWith('/' + btn.textContent.replace('p','') + '/main.m3u8')
    ...
    function setupEvents() {
      ...
      const centralBtn = document.getElementById('centralPlayPause');
      document.getElementById('rewind10').onclick = () => { video.currentTime = Math.max(0, video.currentTime - 10); };
      document.getElementById('forward10').onclick = () => { video.currentTime = Math.min(video.duration, video.currentTime + 10); };
      centralBtn.onclick = () => { video.paused ? video.play() : video.pause(); };
      video.onplay = () => {
        document.getElementById('playPause').textContent = '❚❚';
        centralBtn.textContent = '❚❚';
      };
      video.onpause = () => {
        document.getElementById('playPause').textContent = '►';
        centralBtn.textContent = '►';
      ...
      video.ontimeupdate = () => {
        const displayedTime = video.currentTime - 14;
        const pct = video.duration ? ((video.currentTime - 14) / (video.duration - 14)) * 100 : 0;
        document.getElementById('seek').value = pct;
        const displayedTime2 = video.currentTime - 14;
        document.getElementById('currentTime').textContent = fmt(displayedTime2 > 0 ? displayedTime2 : 0);
      };
      video.onloadedmetadata = () => document.getElementById('duration').textContent = fmt(Math.max(0, video.duration - 14));
      document.getElementById('seek').oninput = e => {
        const dd = video.duration - 14;
        video.currentTime = 14 + (e.target.value / 100) * dd;
      };
